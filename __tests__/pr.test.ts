...
describe('#validate', () => {
  beforeEach(() => {
    ...
    mockClient.prototype.issueExists = jest.fn().mockResolvedValue(true);
  });
  ...
  test('invalid when jira card does not exist', async () => {
    jest
      .spyOn(JiraClientImpl.prototype, 'issueExists')
  ...
  test('invalid when one jira card does not exist', async () => {
    mockClient.prototype.issueExists = jest.fn().mockImplementation((x) =>
